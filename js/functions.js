...
function winner() {
  if (
    ...
    lastLevelMusic.volume = 0;
    lastLevelMusic.pause();
...
function passLevel() {
  if (
    ...
    setTimeout(() => {
      ...
      ctx.fillText(`AND IT IS WORKING!`, 250, 370);
      ctx.fillText(`COVID-19 is losing...`, 250, 420);
...
function gameOver() {
  if (character.lives < 1 && !winner() && !passLevel()) {
    backgroundSound.pause();
    backgroundSound.volume = 0;
    generalMusic.pause();
    generalMusic.volume = 0;
    lastLevelMusic.pause();
    lastLevelMusic.volume = 0;
    lose.play();
    setTimeout(() => {
...
function collisionPedestrian() {
  pedestrians.forEach((pedestrian, index) => {
    if (
      collisionDetecter(pedestrian) &&
      !pedestrian.hasMask &&
...
function generatePedestrians() {
  if (frames % 800 === 0) {
    // modulo 1300 normalmente pero 800 para la demo
